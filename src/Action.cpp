...
void OpenTrainer::act(Studio& studio)
{
    ...
    if (t == nullptr|| t->isOpen()||customers.size()==0){
...
void Order::act(Studio& studio)
{
    ...
    if (t == nullptr || !t->isOpen()) {
        ...
        if (t->getOrders().empty()) {
            ...
            for (unsigned i = 0; i < trainerCustomerl.size(); i++)
...
void MoveCustomer::act(Studio& studio)
{
    ...
    signed dstsize=0;
    if(tdst!=nullptr)
    	dstsize=tdst->getCustomers().size();
    if (tsrc == nullptr || !tsrc->isOpen()|| tdst == nullptr || !tdst->isOpen()|| tsrc->getCustomer(id)==nullptr|| tdst->getCapacity() == dstsize)
...
void PrintTrainerStatus::act(Studio& studio)
{
    ...
    if(t!=nullptr){
        if (!t->isOpen())
            ...
            for (unsigned i = 0;i < t->getCustomers().size();i++) {
                std::cout << t->getCustomers()[i]->getId() <<" "<< t->getCustomers()[i]->getName() << std::endl;
            }
            std::cout << "Orders:" << std::endl;
            for (unsigned i = 0;i < t->getOrders().size();i++) {
