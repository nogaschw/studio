...
Trainer::Trainer(int t_capacity) :id(0),capacity(t_capacity),open(false),customersList(),orderList(),salary(0)
{
}
...
void Trainer::removeCustomer(int id)
{
    ...
    orderList.clear();
    for (unsigned i = 0; i < copyOrder.size(); i++) {
...
Trainer::Trainer(const Trainer& other):id(other.id), capacity(other.capacity), open(other.open),customersList(),orderList(other.orderList),salary(other.salary)
{
...
Trainer::Trainer(Trainer &&other):id(other.id), capacity(other.capacity), open(other.open),customersList(other.customersList),
    orderList(other.orderList),salary(other.salary){
...
void Trainer::addCustomerOrders(std::vector<OrderPair> CustomerOrders) {
    ...
    for(unsigned i=0; i<CustomerOrders.size(); i++){
